...
setlocal autoindent
setlocal indentexpr=GetYueIndent(v:lnum)
" Make sure GetYueIndent is run when these are typed so they can be
" indented or outdented.
setlocal indentkeys+=0],0),0.,=else,=elseif

" Only define the function once.
if exists("*GetYueIndent")
  finish
endif
...
let s:SYNTAX_STRING = 'yue\%(String\|AssignString\|Embed\|Regex\|Heregex\|'
\                   . 'Heredoc\)'

" Syntax names for comments
let s:SYNTAX_COMMENT = 'yue\%(Comment\|BlockComment\|HeregexComment\)'
...
function! s:GetYueIndent(curlinenum)
...
" Wrap s:GetYueIndent to keep the cursor position.
function! GetYueIndent(curlinenum)
  let oldcursor = getpos('.')
  let indent = s:GetYueIndent(a:curlinenum)
